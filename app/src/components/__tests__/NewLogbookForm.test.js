...
describe("NewLogbookForm", () => {
  ...
  it("displays the positionalValidationModal if no positional data is entered", async () => {
    ...
    await waitFor(() => {
      ...
      expect(
        getByText("End logbook without positional data?")
      ).toBeInTheDocument();
      expect(getByText("End logbook")).toBeInTheDocument();
    });
  });
  ...
  it("does not display the positionalValidationModal if GPS mark is entered", async () => {
    ...
    await waitFor(() => {
      ...
      expect(
        queryByText("End logbook without positional data?")
      ).not.toBeInTheDocument();
      expect(queryByText("End logbook")).not.toBeInTheDocument();
    });
  });
});
